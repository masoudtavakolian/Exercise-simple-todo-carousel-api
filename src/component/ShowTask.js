...
export default function ShowTask(props) {
      ...
      finalString =
        ...
        `${toFind
          ...
          .replace(
            " ",
            "&nbsp;"
          )}<span style='text-align:left;color:orange'>${toFind
          .slice(lastIndex, lastIndex + find.length)
          .replace(" ", "&nbsp;")}</span>`;
  ...
  if (props.user != undefined) {
    finalJsxList = finalStringTaskList.map((item, index) => {
      if (item.userId === props.user) {
        countItems++;
        return (
          <div key={index + "firstdiv"} className="groupTask">
            <Checkbox
              key={index + "checkbox"}
            ...
            <div
              ...
              onClick={() => {
                item.description === ""
                  ? alert("No Description Defined!")
                  : alert("Item Description:\n" + item.description);
              }}
            >
              <span key={index + "span"} className="numberList">
                {countItems + " - "}
              </span>
              {item.title}
            </div>
        ...
        return (
          <div key={index + "firstdiv"} className="groupTask">
            ...
            >
              ...
              {ReactHtmlParser(colorShowSearchResult(props.search, item.title))}
            </div>
          </div>
        );
      } else return null;
    });
    if (countItems === 0) messageBelowSearch = props.search + " Not Find!";
  } else if (props.search !== undefined && props.search === "") {
  ...
  return (
    <>
      {countItems > 0 ? (
        <div className="groupTaskContainer">{finalJsxList}</div>
      ) : null}
      {messageBelowSearch !== "" ? (
        <div className="groupTaskContainer">{messageBelowSearch}</div>
      ) : null}
      {messageBelowSearchOther !== "" ? (
        <div className="groupTaskContainer">{messageBelowSearchOther}</div>
      ) : null}
    </>
  );
}
